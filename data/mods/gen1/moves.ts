	...
	bind: {
		...
		onHit(target, source) {
			...
			if (target.volatiles['partiallytrapped']) {
				if (source.volatiles['bindlock'] && source.volatiles['bindlock'].duration > 1) {
	...
	wrap: {
		...
		onHit(target, source) {
			...
			if (target.volatiles['partiallytrapped']) {
				...
				if (source.volatiles['wraplock'] && source.volatiles['wraplock'].duration > 1) {
					target.volatiles['partiallytrapped'].duration = 2;
				}
			}
		},
