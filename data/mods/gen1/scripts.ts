...
export const Scripts: ModdedBattleScriptsData = {
	...
	actions: {
		...
		runMove(moveOrMoveName, pokemon, targetLoc, sourceEffect) {
			...
			if (pokemon.volatiles['wraplock']){
				move = this.battle.dex.getActiveMove('wrap');
			}
			...
			if (
				(!lockedMove && (!pokemon.volatiles['partialtrappinglock']  || pokemon.volatiles['partialtrappinglock'].locked !== target && !pokemon.volatiles['wraplock'] || pokemon.volatiles['wraplock'].locked !== target))
			) {
		...
		useMove(moveOrMoveName, pokemon, target, sourceEffect) {
			...
			let baseMove = this.battle.dex.moves.get(moveOrMoveName);
			/*if (pokemon.volatiles['wraplock']){
				baseMove = this.battle.dex.moves.get("wrap");
			}*/
			let move = this.battle.dex.getActiveMove(baseMove);
			if (target === undefined) {
				console.log(move);
				console.log(this.battle.getRandomTarget(pokemon, move));
			...
			if (move.id !== 'metronome') {
				if (move.id !== 'mirrormove' ||
					...
					if (target && target.hp <= 0) {
						...
						delete pokemon.volatiles['wraplock'];
						delete pokemon.volatiles['bindlock'];
					} else {
					...
					if (move.volatileStatus === 'partiallytrapped' && target && target.hp > 0) {
						...
						if ((pokemon.volatiles['partialtrappinglock'] || pokemon.volatiles['wraplock'] || pokemon.volatiles['bindlock']) && target.volatiles['partiallytrapped']) {
							// Here the partialtrappinglock volatile has been already applied
							let sourceVolatile = pokemon.volatiles['wraplock'];
		...
		useMoveInner(moveOrMoveName, pokemon, target, sourceEffect) {
			...
			if (pokemon.volatiles['bindlock']){
				move = this.battle.dex.getActiveMove('bind');
			}
		...
		tryMoveHit(target, pokemon, move) {
			...
			if (move.volatileStatus === 'partiallytrapped' && (target === pokemon.volatiles['partialtrappinglock']?.locked || target === pokemon.volatiles['wraplock']?.locked || target === pokemon.volatiles['bindlock']?.locked)) {
				accuracy = true;
			}
		...
		moveHit(target, pokemon, move, moveData, isSecondary, isSelf) {
			...
			if (target) {
				...
				if (targetSub && moveData.volatileStatus && moveData.volatileStatus === 'partiallytrapped') {
					...
					if (!pokemon.volatiles['partialtrappinglock'] && ((!pokemon.volatiles['wraplock'] || pokemon.volatiles['wraplock'].duration > 1))|| pokemon.volatiles['partialtrappinglock'].duration > 1) {
			...
			if (moveData.self && (moveData.self.volatileStatus !== 'lockedmove') &&
				(doSelf || moveData.self.volatileStatus === 'partialtrappinglock' || moveData.self.volatileStatus === 'wraplock' || moveData.self.volatileStatus === 'bindlock')) {
			...
			if (pokemon.volatiles['wraplock']) {
				console.log("hi guys" + pokemon.volatiles)
				pokemon.volatiles['wraplock'].damage = this.battle.lastDamage;
			}
		...
		getDamage(source, target, move, suppressMessages) {
			...
			if (source.volatiles['wraplock'] && (target === source.volatiles['wraplock'].locked)) {
				return source.volatiles['wraplock'].damage;
			}
